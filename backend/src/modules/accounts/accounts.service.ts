...
export class AccountsService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    Object.values(Currency).forEach(async currency => {
      let account = await this.accountRepo.findOneBy({ type: AccountType.ASSET, currency })
      if (!account) {
  ...
  async createAndCreditInitialAccounts(manager: EntityManager, userId: number) {
    const accountsInsertResult = await manager.createQueryBuilder(Account, 'account')
      .insert()
      .values([
    ...
    const transactionsInsertResult = await manager.createQueryBuilder()
      .insert()
    ...
    await Promise.all(
      transactions.map(tx => manager.query(
    ...
    await manager.insert(Ledger, transactions.flatMap(tx => ([
      {
